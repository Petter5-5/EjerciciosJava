...
public class Bala extends Enemigo
{
    ...
    private int x = 0;
    private int y = 0;
    private int movimietno = 7;
    private double angleIn;
    
    public Bala(int x, int y, Jugador j1, Jugador j2)
    {
        this.x = x;
        this.y = y;
        ...
        int cx = x + ancho / 2;
                ...
                int targetX = (dist1 <= dist2) ? cx1 : cx2;
                int targetY = (dist1 <= dist2) ? cy1 : cy2;
    ...
    {
        if (j1.isDeath() && j2.isDeath()) return;
        x += (int)(Math.cos(angleIn) * 7);
        y += (int)(Math.sin(angleIn) * 7);
    }
